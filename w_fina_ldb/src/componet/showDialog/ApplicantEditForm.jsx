...
const ApplicantEditForm = ({ open, onOpenChange, report, provinces, districts, setDistricts, setSelectedProvinceId, fetchReports }) => {
  ...
  useEffect(() => {
    const fetchApplicant = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${url.base_url}/api/applicant/${applicant_id}`, {
    ...
    fetchApplicant();
  }, [applicant_id, setSelectedProvinceId]);
  ...
  const handleDateChange = (e) => {
    ...
    if (value && !/^\d{2}\/\d{2}\/\d{4}$/.test(value)) {
      setError(`ຮູບແບບວັນທີ່ບໍ່ຖືກຕ້ອງ ສຳລັບ ${name === 'dob' ? 'ວັນເດືອນປີເກີດ' : name === 'issued_date' ? 'ວັນທີ່ອອກ' : 'ວັນທີ່ໝົດອາຍຸ'}: ໃຊ້ dd/mm/yyyy`);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${url.base_url}/api/delete-document/${applicant_id}/${fileToDelete}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleCancelMainDialog = () => {
    const validationError = validateForm();
    if (validationError) {
      setError(validationError);
      setDialogTitle('ຂໍ້ຜິດພາດ');
      setDialogMessage(`ກະລຸນາປ້ອນຂໍ້ມູນທີ່ບັງຄັບໃຫ້ຄົບຖ້ວນກ່ອນອອກ: ${validationError}`);
      setIsErrorDialogOpen(true);
      return;
    }
    onOpenChange(false);
  ...
  const handleCancelConfirmDialog = () => {
    ...
    if (validationError) {
      setError(validationError);
      setDialogTitle('ຂໍ້ຜິດພາດ');
      setDialogMessage(`ກະລຸນາປ້ອນຂໍ້ມູນທີ່ບັງຄັບໃຫ້ຄົບຖ້ວນກ່ອນຍົກເລີກ: ${validationError}`);
      setIsErrorDialogOpen(true);
      return;
    }
  ...
  const handleSubmission = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.put(`${url.base_url}/api/update-applicant/${applicant_id}`, data, {
        headers: {
      ...
      const updatedResponse = await axios.get(`${url.base_url}/api/applicant/${applicant_id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <>
      {/* Main Dialog - fullscreen to avoid modal stacking issues */}
      <Dialog open={open} onOpenChange={handleDialogClose}>
        <DialogContent className="fixed inset-0 w-full h-full max-w-full max-h-full m-0 p-0 rounded-none border-none overflow-y-auto font-noto-sans-lao bg-slate-50">
          ...
          <div className="p-6">
            ...
            <CardContent>
              {isLoading && <div className="text-gray-600 mb-4">ກຳລັງໂຫຼດຂໍ້ມູນ...</div>}
          ...
          <DialogFooter className="mt-4 sm:flex sm:flex-row sm:justify-end gap-4">
            <Button
              variant="outline"
              onClick={handleCancelMainDialog}
