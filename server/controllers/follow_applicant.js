...
exports.getDataEntryReport = (req, res) => {
...
exports.getReport = (req, res) => {
  ...
  // ✅ ເກົ່າສຸດຢູ່ເທີງ
  query += ` ORDER BY a.updated_at ASC, a.id ASC LIMIT ? OFFSET ?`;
  queryParams.push(limitNum, offset);
...
exports.getVerifierReport = (req, res) => {
  ...
  // ✅ ແກ້ຢູ່ນີ້ — filter status ຄັ້ງດຽວ
  if (status) {
    query += ` AND a.current_status = ?`;
    queryParams.push(status);
  } else {
    query += ` AND a.current_status IN ('in_progress', 'rejected')`;
  }
  ...
  db.query(query, queryParams, (err, results) => {
    ...
    db.query(docQuery, [applicantIds], (docErr, docResults) => {
      ...
      // ✅ ແກ້ countQuery ຄືກັນ
      if (status) {
        countQuery += ` AND a.current_status = ?`;
        countParams.push(status);
      } else {
        countQuery += ` AND a.current_status IN ('in_progress', 'rejected')`;
      }
